...
public class Filter3 extends Filters {
	...
	public void run(String id,String item,ArrayList<String> FilterResult) throws Exception {
		...
		if(sellnumberslast > sellnumbersnow) {
			...
			String coeres=(res[0] > 100)?"<h4 class=\"text-danger\">"+"SELL(DW) COEVar "+String.valueOf(res[0])+"</h4>":"COEVar "+String.valueOf(res[0]);
			String speed=(res[1] > 50)?"<hr /><h4 class=\"text-danger\">"+"SELL(DW) Speed [Queue] "+String.valueOf(res[1])+"</h4>":"<hr/>Speed "+String.valueOf(res[1]);
			...
			double[] res=CalculateCOEVariationAndGrowthSpeed(2,InRamDBDeep);
			String coeres=(res[0] > 100)?"<h4 class=\"text-danger\">"+"SELL(UP) COEVar "+String.valueOf(res[0])+"</h4>":"COEVar "+String.valueOf(res[0]);
			String speed=(res[1] > 50)?"<hr /><h4 class=\"text-danger\">"+"SELL(UP) Speed [Queue] "+String.valueOf(res[1])+"</h4>":"<hr/>Speed "+String.valueOf(res[1]);
			...
			String coeres=(res[0] > 100)?"<h4 class=\"text-danger\">"+"BUY(DW) COEVar "+String.valueOf(res[0])+"</h4>":"COEVar "+String.valueOf(res[0]);
			String speed=(res[1] > 50)?"<hr /><h4 class=\"text-danger\">"+"BUY(DW) Speed [Queue] "+String.valueOf(res[1])+"</h4>":"<hr/>Speed "+String.valueOf(res[1]);

			Result+="BUYER DW "+String.valueOf(buynumberslast-buynumbersnow)+" "+coeres+" "+speed+" CSS{p-3@mb-2@bg-danger@text-white}";
		}
		else if (buynumberslast < buynumbersnow) {
			double[] res=CalculateCOEVariationAndGrowthSpeed(3,InRamDBDeep);
			String coeres=(res[0] > 100)?"<h4 class=\"text-danger\">"+"BUY(UP) COEVar "+String.valueOf(res[0])+"</h4>":"COEVar "+String.valueOf(res[0]);
			String speed=(res[1] > 50)?"<hr /><h4 class=\"text-danger\">"+"BUY(UP) Speed [Queue] "+String.valueOf(res[1])+"</h4>":"<hr/>Speed "+String.valueOf(res[1]);
			
			Result+="BUYER UP "+String.valueOf(buynumbersnow-buynumberslast)+" "+coeres+" "+speed+" CSS{p-3@mb-2@bg-success@text-white}";
			
			//Need Flow So ADDED//
			int resscore=(res[0] >= 1000)?ScoreAssign(id, "COEVar-Buyer","COEVar-Buyer FIRE", this.PointX*1,ScoreType.ADDED):
				((res[0] >= 600)?ScoreAssign(id, "COEVar-Buyer","COEVar-Buyer BURST", this.PointX*1,ScoreType.ADDED): // COEVar More Impress If More Than 150 COEVar //
			((res[0] >= 200)?ScoreAssign(id, "COEVar-Buyer","COEVar-Buyer UP", this.PointX*1,ScoreType.ADDED):0)); // COEVar More Impress If More Than 150 COEVar //
			
			resscore=(res[1] >= 100)?ScoreAssign(id, "SPEED-Buyer","SPEED-Buyer QUEUE", this.PointX*1,ScoreType.ADDED): // COEVar More Impress If More Than 150 COEVar //
				((res[1] >= 30)?ScoreAssign(id, "SPEED-Buyer","SPEED-Buyer BURST", this.PointX*1,ScoreType.ADDED):
				(res[1] >= 20)?ScoreAssign(id, "SPEED-Buyer","SPEED-Buyer UP", this.PointX*1,ScoreType.ADDED):0); // COEVar More Impress If More Than 150 COEVar //
		}
		else
			Result+="FIX";
